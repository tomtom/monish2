...
function fmtReset(w) {
    ...
    return dt ? ' (reset: ' + dt.format(DATE_FMT) + ')' : '';
}

print('Rolling: ' + pct(five)  + fmtReset(five));
print('Weekly: '  + pct(seven) + fmtReset(seven));`;
...
try {
    ...
    print('Activity: $' + totalSpend.toFixed(2));
...
Option 2 — Persistent udev rule (survives reboots):
  ...
  sudo udevadm trigger`,
    ...
    {
        name:               'Claude Usage',
        description:        'Claude Code rolling (5 h) and weekly (7 d) utilisation from the Anthropic OAuth usage API. Requires an active Claude Code login (~/.claude/.credentials.json). CAUTION ≥75 %, DANGER ≥90 %.',
...
and complete the login flow.  The credentials are stored at:
  ...
  ~/.claude/.credentials.json`,
        ...
        showSparkline:      false,
        args:               [
            {name: 'DATE_FMT', label: 'Reset date format', default: '%a %d %H:%M'},
        ],
    },
    {
        name:            'OpenRouter',
