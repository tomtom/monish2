...
export const PRESET_MONITORS = [
    ...
    // ---- Processes ---------------------------------------------------------
    {
        ...
        // split($11,a,"/") extracts the basename; one process per line for readability
        command:         'ps aux --sort=-%cpu 2>/dev/null | awk \'NR>=2 && NR<=4 {n=split($11,a,"/"); printf "%s %.0f%%\\n", a[n], $3}\'',
        intervalSeconds: PRESET_MIN_INTERVAL,
        outputRegex:     '',
        cautionPatterns: [],
        dangerPatterns:  [],
    ...
    },
    {
        ...
        // split($11,a,"/") extracts the basename; one process per line for readability
        command:         'ps aux --sort=-%mem 2>/dev/null | awk \'NR>=2 && NR<=4 {n=split($11,a,"/"); printf "%s %.0f%%\\n", a[n], $4}\'',
        intervalSeconds: PRESET_MIN_INTERVAL,
        outputRegex:     '',
        cautionPatterns: [],
        dangerPatterns:  [],
