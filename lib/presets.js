...
/**
 * GJS JavaScript that runs `grdctl status` and extracts the RDP state.
 * `grdctl` is part of gnome-remote-desktop (GNOME 42+).
 *
 * When enabled, also counts active :3389 connections via `ss -tnp` (checked
 * once per run via GLib.find_program_in_path so no extra process is spawned
 * when ss is absent).
 *
 * Output: "disabled", "enabled", "enabled (N)", or "N/A".
 */
const GNOME_RDP_JS = `\
try {
    ...
    if (!ok || !out) { print('N/A'); } else {
        ...
        if (!m) { print('N/A'); } else {
            const rdpStatus = m[1];
            if (rdpStatus !== 'enabled') {
                print(rdpStatus);
            } else {
                const ssPath = GLib.find_program_in_path('ss');
                if (!ssPath) { print('enabled'); } else {
                    const [ssOk, ssOut] = GLib.spawn_command_line_sync('ss -tnp');
                    const lines = (ssOk && ssOut)
                        ? new TextDecoder().decode(ssOut).split('\\n')
                        : [];
                    const count = lines.filter(l => l.includes(':3389')).length;
                    print('enabled (' + count + ')');
                }
            }
        }
...
Option 2 — Persistent udev rule (survives reboots):
  ...
  sudo udevadm trigger`,
    ...
    {
        name:            'Gnome RDP',
        description:     'GNOME Remote Desktop RDP status. CAUTION when enabled; DANGER when active connections exist (requires ss).',
        command:         GNOME_RDP_JS,
        type:            MonitorType.JAVASCRIPT,
        intervalSeconds: PRESET_MIN_INTERVAL,
        outputRegex:     '',
        cautionPatterns: ['enabled'],
        dangerPatterns:  ['enabled \\([1-9]'],
        enabled:         true,
        actions:         [
            {label: 'Enable',  command: 'grdctl rdp enable',  type: MonitorType.SHELL, guard: "value === 'disabled'"},
            {label: 'Disable', command: 'grdctl rdp disable', type: MonitorType.SHELL, guard: "value.startsWith('enabled')"},
        ],
    },
