...
function thermalJS(zone) {
    return `\
const [ok, b] = GLib.file_get_contents('/sys/class/thermal/thermal_zone${zone}/temp');
if (!ok) { print('N/A'); } else {
...
function _readFile(path) {
    const [ok, b] = GLib.file_get_contents(path);
    return ok ? new TextDecoder().decode(b).trim() : null;
}
function _listPids() {
    const pids = [];
    try {
