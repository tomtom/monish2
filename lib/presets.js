...
/**
 * GJS JavaScript that reads /var/run/utmp-equivalent via the `who` command
 * (part of GNU coreutils on all major distros) and prints unique usernames,
 * comma-separated, on a single line.
 *
 * Uses GLib.spawn_command_line_sync so no external executor is needed and the
 * script stays dependency-free. Falls back to "N/A" when who is unavailable.
 */
...
try {
    ...
    if (!ok || !out) { print('N/A'); } else {
        ...
        print(names.length > 0 ? names.join(', ') : 'none');
    }
} catch (_) { print('N/A'); }`;
...
export const PRESET_MONITORS = [
    ...
    // ---- Processes ---------------------------------------------------------
    {
        ...
        dangerPatterns:  [],
        enabled:         true,
        args:            [{name: 'EXCLUDE', label: 'Exclude regexp', default: '^(gjs)$'}],
    },
    {
