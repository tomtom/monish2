...
export const PRESET_MONITORS = [
    ...
    {
        name:            'CPU Power (RAPL)',
        description:     'Estimated CPU package power draw in watts via Intel RAPL (samples /sys/class/powercap/intel-rapl/intel-rapl:0/energy_uj over 0.5 s). Requires read access to energy_uj — enable once without sudo: sudo chmod a+r /sys/class/powercap/intel-rapl/intel-rapl:0/energy_uj; or add a udev rule (see RAPL_POWER_JS comment in presets.js).',
    ...
    // ---- Remote ------------------------------------------------------------
    {
        ...
        enabled:         true,
        actions:         [
            {label: 'Enable',  command: 'grdctl rdp enable',  type: MonitorType.SHELL},
            {label: 'Disable', command: 'grdctl rdp disable', type: MonitorType.SHELL},
        ],
    },
