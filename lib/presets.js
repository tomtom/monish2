...
// ---------------------------------------------------------------------------
// AI-agent process check interval expression
// ---------------------------------------------------------------------------

/**
 * GJS interval expression that checks whether any process whose /proc/[pid]/comm
 * basename matches the AI_AGENT regexp is currently running.
 *
 * Returns 900 (15 minutes) when a match is found so the monitor polls on that
 * cadence; returns 0 when no match is found so the scheduler skips the API call
 * and re-checks after EXPR_RECHECK_MS (60 s).
 *
 * AI_AGENT is injected by the scheduler as a const declaration from argValues.
 */
const AI_AGENT_INTERVAL_JS = `\
const Gio = imports.gi.Gio;
const _pattern = new RegExp(AI_AGENT);
let _running = false;
try {
    const _en = Gio.File.new_for_path('/proc').enumerate_children('standard::name,standard::type', 0, null);
    let _info;
    while (!_running && (_info = _en.next_file(null)) !== null) {
        if (_info.get_file_type() !== Gio.FileType.DIRECTORY || !/^\\d+$/.test(_info.get_name())) continue;
        try {
            const [, _b] = GLib.file_get_contents('/proc/' + _info.get_name() + '/comm');
            if (_pattern.test(new TextDecoder().decode(_b).trim())) _running = true;
        } catch (_) {}
    }
} catch (_) {}
print(_running ? 900 : 0);`;
...
try {
    ...
    if (msg.get_status() !== 200) { print('HTTP ' + msg.get_status()); return; }
...
function fmtReset(w) {
    if (!w || typeof w !== 'object') return '';
    const r = w.resets_at ?? w.reset_at ?? w.resetsAt ?? null;
    if (!r) return '';
    let dt;
    if (typeof r === 'number') {
        const secs = r > 1e12 ? Math.floor(r / 1000) : r;
        dt = GLib.DateTime.new_from_unix_local(secs);
    } else {
        dt = GLib.DateTime.new_from_iso8601(String(r), null);
        if (dt) dt = dt.to_local();
    }
    return dt ? ' (reset: ' + dt.format('%H:%M') + ')' : '';
}

const lbl5 = '5h' + fmtReset(five);
const lbl7 = '7d' + fmtReset(seven);
const w = Math.max(lbl5.length, lbl7.length);
print(lbl5.padEnd(w + 6) + pct(five));
print(lbl7.padEnd(w + 6) + pct(seven));`;
...
try {
    ...
    print('Activity (30d): $' + totalSpend.toFixed(2));
...
Option 2 — Persistent udev rule (survives reboots):
  ...
  sudo udevadm trigger`,
    ...
    // ---- Remote ------------------------------------------------------------
    {
        ...
        enabled:         true,
        actions:         [
            {label: 'Enable',  command: 'grdctl rdp enable',  type: MonitorType.SHELL, guard: "matches 'disabled'"},
            {label: 'Disable', command: 'grdctl rdp disable', type: MonitorType.SHELL, guard: "matches 'enabled'"},
        ],
    },
...
and complete the login flow.  The credentials are stored at:
  ...
  ~/.claude/.credentials.json`,
        ...
        type:               MonitorType.JAVASCRIPT,
        intervalSeconds:    900,
        intervalExpression: AI_AGENT_INTERVAL_JS,
        outputRegex:        '',
        ...
        showSparkline:      false,
        args:               [
            {name: 'AI_AGENT', label: 'AI Agent regexp', default: '^(claude)$'},
        ],
    },
    {
        name:            'OpenRouter',
...
Provide it via one of:
  ...
  echo sk-or-v1-… > ~/.config/openrouter/key`,
        ...
        showSparkline:      false,
        args:               [
            {name: 'AI_AGENT', label: 'AI Agent regexp', default: '^(opencode)$'},
        ],
    },
