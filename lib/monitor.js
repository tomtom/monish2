...
/**
 * Fill in any missing fields on a stored monitor with safe defaults.
 * Runs automatically on every deserialization so old monitors created before a
 * schema addition (e.g. actions.guard added in ISSUE 64) can still be opened
 * in the edit dialog without crashing.
 * Existing field values are never overwritten.
 *
 * @param {object} m - Raw monitor object from stored JSON.
 * @returns {object} Fully-normalised monitor with all required fields present.
 */
export function normalizeMonitor(m) {
    const id = m.id ?? generateId();
    const normalizedActions = Array.isArray(m.actions)
        ? m.actions.map(a => ({
            label:   a.label   ?? '',
            command: a.command ?? '',
            type:    a.type    ?? MonitorType.SHELL,
            guard:   a.guard   ?? '',
        }))
        : [];
    const normalizedArgs      = Array.isArray(m.args) ? m.args : [];
    const normalizedArgValues = (m.argValues && typeof m.argValues === 'object') ? {...m.argValues} : {};
    const intervalSec         = m.intervalSeconds ?? 60;
    // Populate argValues from arg `default` fields when not already set.
    for (const arg of normalizedArgs) {
        if (arg.name && normalizedArgValues[arg.name] === undefined && arg.default !== undefined) {
            normalizedArgValues[arg.name] = arg.default;
        }
    }
    return {
        id,
        ...
        helpText:           m.helpText           ?? '',
        command:            m.command            ?? '',
        type:               m.type               ?? MonitorType.SHELL,
        intervalSeconds:    intervalSec,
        outputRegex:        m.outputRegex        ?? '',
        cautionPatterns:    Array.isArray(m.cautionPatterns) ? m.cautionPatterns : [],
        dangerPatterns:     Array.isArray(m.dangerPatterns)  ? m.dangerPatterns  : [],
        enabled:            m.enabled            ?? true,
        onDemand:           m.onDemand           ?? (intervalSec === 0),
        args:               normalizedArgs,
        argValues:          normalizedArgValues,
        actions:            normalizedActions,
        showSparkline:      m.showSparkline       ?? true,
    };
}

/**
 * Deserialize a JSON string (from GSettings) into an array of monitor objects.
 * Each monitor is passed through normalizeMonitor() to fill in fields that were
 * absent in older schema versions.  Returns an empty array on any parse error.
 *
 * @param {string} json
 * @returns {object[]}
 */
export function deserializeMonitors(json) {
    ...
    try {
        ...
        return raw.map(normalizeMonitor);
    } catch (_) {
        return [];
    }
}
...
/**
 * Evaluate an action guard expression against the current monitor value.
 * Implements a restricted safe mini-language — no dynamic code execution.
 *
 * Supported forms:
 *   matches /pattern/flags   — regex test on value
 *   not matches /pattern/f   — negated regex test
 *   matches 'str'            — exact string equality (single or double quotes)
 *   not matches 'str'        — exact string inequality
 *
 * Legacy JS forms also accepted for backward compatibility:
 *   value === 'str'          — exact equality
 *   value !== 'str'          — exact inequality
 *
 * Any unrecognised expression returns true (button always visible).
 *
 * @param {string} guard - Guard expression string.
 * @param {string} value - Current monitor value to test against.
 * @returns {boolean}
 */
export function evaluateGuard(guard, value) {
    if (!guard || !guard.trim()) return true;
    const g = guard.trim();

    // matches /pattern/flags  OR  not matches /pattern/flags
    let m = g.match(/^(not\s+)?matches\s+\/(.+)\/([gimsuy]*)\s*$/);
    if (m) {
        try {
            const hit = new RegExp(m[2], m[3]).test(value);
            return m[1] ? !hit : hit;
        } catch (_) { return true; }
    }

    // matches 'str'  OR  not matches 'str'  (single or double quotes)
    m = g.match(/^(not\s+)?matches\s+(['"])(.+)\2\s*$/);
    if (m) {
        const hit = value === m[3];
        return m[1] ? !hit : hit;
    }

    // Legacy: value === 'str'  OR  value !== 'str'
    m = g.match(/^value\s*(===|!==)\s*(['"])(.+)\2\s*$/);
    if (m) return m[1] === '===' ? value === m[3] : value !== m[3];

    return true; // unrecognised → always show
}

/**
 * Parse a serialised guard string into a structured {op, pattern} pair for UI use.
 *
 * Op values: '' (always), 'matches', 'not matches', 'matches-re', 'not matches-re'.
 * Regex flags are dropped — the UI does not expose them.
 * Legacy JS forms (value === / !==) are normalised to 'matches' / 'not matches'.
 * Unrecognised expressions return op=''.
 *
 * @param {string|null|undefined} guard
 * @returns {{op: string, pattern: string}}
 */
export function parseGuard(guard) {
    if (!guard || !guard.trim()) return {op: '', pattern: ''};
    const g = guard.trim();

    let m = g.match(/^(not\s+)?matches\s+\/(.+)\/[gimsuy]*\s*$/);
    if (m) return {op: m[1] ? 'not matches-re' : 'matches-re', pattern: m[2]};

    m = g.match(/^(not\s+)?matches\s+(['"])(.+)\2\s*$/);
    if (m) return {op: m[1] ? 'not matches' : 'matches', pattern: m[3]};

    m = g.match(/^value\s*(===|!==)\s*(['"])(.+)\2\s*$/);
    if (m) return {op: m[1] === '===' ? 'matches' : 'not matches', pattern: m[3]};

    return {op: '', pattern: ''};
}

/**
 * Serialise a structured guard {op, pattern} back to the canonical guard string.
 *
 * @param {string} op      - One of '', 'matches', 'not matches', 'matches-re', 'not matches-re'.
 * @param {string} pattern - The literal string or regex pattern (no quotes/slashes).
 * @returns {string}
 */
export function serializeGuard(op, pattern) {
    if (!op || !pattern || !pattern.trim()) return '';
    if (op === 'matches')         return `matches '${pattern}'`;
    if (op === 'not matches')     return `not matches '${pattern}'`;
    if (op === 'matches-re')      return `matches /${pattern}/`;
    if (op === 'not matches-re')  return `not matches /${pattern}/`;
    return '';
}
...
// ---------------------------------------------------------------------------
// On-demand time helpers
// ---------------------------------------------------------------------------

/**
 * Format a duration in milliseconds as a short human-readable age string.
 * Used to show how long ago an on-demand monitor value was collected.
 *
 * Returns "just now" for durations under 60 s, "N min ago" for under 60 min,
 * and "N h ago" for anything longer.
 *
