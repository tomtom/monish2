...
129. Warning in make shexli:
    EGO-M-005  warning
    Field `session-modes` should be omitted when it only contains `user`.

      --> monish2@thm.link.shell-extension.zip:metadata.json

      = rule: metadata session-modes must be omitted when only user mode is declared
      = help: metadata.json must be well-formed
        https://gjs.guide/extensions/review-guidelines/review-guidelines.html#metadata-json-must-be-well-formed
